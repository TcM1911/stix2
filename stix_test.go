...
func TestFromJSONAll(t *testing.T) {
	...
	assert.NotNil(c.Incidents())
	assert.NotNil(c.Incident(Identifier("incident--27ba1a3f-c895-4d63-a5cd-4509e6b5966f")))
	assert.Nil(c.Incident(Identifier("")))
...
func TestFromReader(t *testing.T) {
	...
	t.Run("should not fail if empty slice provided", func(t *testing.T) {
		c, err := FromReader(strings.NewReader("[]"))
		require.NoError(t, err)
		require.NotNil(t, c)
	})

	t.Run("should not fail if empty object provided", func(t *testing.T) {
		c, err := FromReader(strings.NewReader("{}"))
		require.NoError(t, err)
		require.NotNil(t, c)
	})
	...
	t.Run("do-not-keep-order", func(t *testing.T) {
		f.Seek(0, 0)
		c, err := FromReader(f, NoSortOption())
		assert.NoError(err)
		assert.NotNil(c)
...
func TestFromReaderAll(t *testing.T) {
	assert := assert.New(t)
	f, err := getResource("all.json")
	require.NoError(t, err)
	defer f.Close()
